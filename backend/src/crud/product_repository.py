...
from src.models.product import ProductOut, ProductIn
...
class ProductRepository(AbstractRepository[ProductIn, ProductDBModel, ProductOut]):
    ...
    async def get_one(self, key: str) -> ProductOut:
        """Get one product by UUID"""
        result = await self.db_session.execute(
            select(ProductDBModel).where(ProductDBModel.id == UUID(str(key), version=4))
        )
